...
const EditResume = () => {
  ...
  // take back to previous state or take back to your first page
  const goBack = () => {
  ...
  // adding new items for info & remove
  ...
  const uploadResumeImages = async () => {
    try {
      ...
      const blob = await domtoimage.toBlob(thumbnailElement);

      const thumbnailFile = new File([blob], `thumbnail-${resumeId}.png`, {
        type: "image/png",
      });
//
  ...
  // all this function will be used for edit resume
  return (
    <DashboardLayout>
              ...
              <div className="flex flex-wrap items-center justify-end gap-3">
                ...
                <button className={buttonStyles.save} onClick={uploadResumeImages} disabled={isLoading}>
                  {isLoading ? (
                    ...
                    <>
                      <Save size={16} />
                      <span className="ml-2">Save & Exit</span>
                    </>
                  )}
                </button>
      ...
      {/* Hidden thumbnail DOM for image generation */}
